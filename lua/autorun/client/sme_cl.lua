...
net.Receive("SMENetworkSoundPatchPlay", function()
    ...
    local flag = net.ReadUInt(11)
    
    if not IsValid(ent) then return end
    
    -- CHAN_AUTO HAS to be here, since default channel can be either CHAN_AUTO or CHAN_WEAPON.
    -- By not setting it, a channel mismatch can occur. This makes the sound impossible to stop with networked CSoundPatch:Stop().
    ent:EmitSound(snd, level, pitch, vol, CHAN_AUTO, SND_STOP)
    ent:EmitSound(snd, level, pitch, vol, CHAN_AUTO, flag, dsp)
end)
...
net.Receive("SMENetworkSoundPatchStop", function()
    ...
    ent:EmitSound(snd, 75, 100, 1, CHAN_AUTO, SND_STOP)
end)

hook.Add("EntityRemoved", "SMEEntRemoved", function(ent, fullUpdate)
    if fullUpdate then return end

    -- CHAN_VOICE seems to be able to exist without parent entity so we need to stop it here.
